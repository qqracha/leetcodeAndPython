...
- <span class="badge">Переиспользование кода</span> — базовый функционал пишется один раз в родительском классе.
- <span class="badge">Расширение (Extending)</span> — дочерний класс добавляет свои уникальные методы и данные.
- <span class="badge">Переопределение (Overriding)</span> — дочерний класс может изменить поведение метода родителя.
...
Посмотреть порядок поиска методов в сложном дереве наследования можно через атрибут ClassName.__mro__.
## Пример
